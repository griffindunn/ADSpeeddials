...
var x = (u, c, e) => c in u ? v(u, c, { enumerable: !0, configurable: !0, writable: !0, value: e }) : u[c] = e;
var p = (u, c, e) => x(u, typeof c != "symbol" ? c + "" : c, e);
import { Desktop as o } from "@wxcc-desktop/sdk";
const t = o.logger.createLogger("cisco-conference-speed-dial");
class w extends HTMLElement {
  constructor() {
    super();
    p(this, "buttonStateMap", null);
    p(this, "initialized", !1);
    p(this, "lastButtonPressTime", 0);
    p(this, "latestCallState", "unknown");
    t.info("lee-health-wxcc-consult-buttons version 1.0"), t.info("JS Step 1 - attachShadow"), this.attachShadow({ mode: "open" }), t.debug("Requesting Button Map Update from background process in constructor... ");
  }
  async connectedCallback() {
    t.debug("*/*/*/*/*/*/*/*/*/*/*/*/*/*/ Connected Callback */*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/*/"), t.debug("connectedCallback in UI Widget"), this.initialized === !1 ? (t.debug("Not previously initialized - doing Desktop.config.init()"), await o.config.init()) : (t.debug("Previously initialized - skipping Desktop.config.init()"), this.removeButtonPressListener()), t.debug("Removing Old Event Listeners"), this.removeListeners(), t.debug("Initializing Event Listeners"), this.initListeners(), t.debug("Adding Button Press Listener"), this.addButtonPressListener(), t.debug("Getting Call State"), this.latestCallState = await this.getCallState(), t.debug("Rendering template"), this.render(), this.initialized = !0;
  }
  ...
  initListeners() {
    t.info("Adding WebexCC Event Listeners"), o.agentContact.addEventListener("eAgentContact", (e) => this.logEvent(e, "eAgentContact")), o.agentContact.addEventListener("eAgentContactAssigned", (e) => this.logEvent(e, "eAgentContactAssigned")), o.agentContact.addEventListener("eAgentContactAssignFailed", (e) => this.logEvent(e, "eAgentContactAssignFailed")), o.agentContact.addEventListener("eAgentContactEnded", (e) => this.logEvent(e, "eAgentContactEnded")), o.agentContact.addEventListener("eAgentContactWrappedUp", (e) => this.logEvent(e, "eAgentContactWrappedUp")), o.agentContact.addEventListener("eAgentOfferContact", (e) => this.logEvent(e, "eAgentOfferContact")), o.agentContact.addEventListener("eAgentOfferContactRona", (e) => this.logEvent(e, "eAgentOfferContactRona")), o.agentContact.addEventListener("eAgentOfferConsult", (e) => this.logEvent(e, "eAgentOfferConsult")), o.agentContact.addEventListener("eAgentWrapup", (e) => this.logEvent(e, "eAgentWrapup")), o.agentContact.addEventListener("eAgentContactHeld", (e) => this.logEvent(e, "eAgentContactHeld")), o.agentContact.addEventListener("eAgentContactUnHeld", (e) => this.logEvent(e, "eAgentContactUnHeld")), o.agentContact.addEventListener("eConsultTransfer", (e) => this.logEvent(e, "eConsultTransfer")), o.agentContact.addEventListener("eAgentConsultCreated", (e) => this.logEvent(e, "eAgentConsultCreated")), o.agentContact.addEventListener("eAgentConsultConferenced", (e) => this.logEvent(e, "eAgentConsultConferenced")), o.agentContact.addEventListener("eAgentConsultEnded", (e) => this.logEvent(e, "eAgentConsultEnded")), o.agentContact.addEventListener("eAgentConsultConferenceEnded", (e) => this.logEvent(e, "eAgentConsultConferenceEnded")), o.agentContact.addEventListener("eAgentConsulting", (e) => this.logEvent(e, "eAgentConsulting")), o.agentContact.addEventListener("eAgentConsultFailed", (e) => this.logEvent(e, "eAgentConsultFailed")), o.agentContact.addEventListener("eAgentConsultEndFailed", (e) => this.logEvent(e, "eAgentConsultEndFailed")), o.agentContact.addEventListener("eAgentConsultConferenceEndFailed", (e) => this.logEvent(e, "eAgentConsultConferenceEndFailed")), o.agentContact.addEventListener("eParticipantLeftConference", (e) => this.logEvent(e, "eParticipantLeftConference")), o.agentContact.addEventListener("eParticipantJoinedConference", (e) => this.logEvent(e, "eParticipantJoinedConference")), o.agentContact.addEventListener("eContactUpdated", (e) => this.logEvent(e, "eContactUpdated"));
  }
  async logEvent(e, a) {
    t.debug("Start logEvent"), t.debug(`************** LOGGING EVENT - ${a} ****************`), t.debug(e);
    let i = await this.getCallStateFromEvent(e);
    i === "terminated" && (i = await this.getCallState()), t.debug("Updating Buttons"), await this.updateButtons(i), t.debug("*********************************************");
  }
  removeListeners() {
    t.info("Removing WebexCC Event Listeners"), o.agentContact.removeAllEventListeners();
  }
  async getCallState() {
    const e = this.details, a = await this.getInteractionId();
    t.debug(`Getting Call State for interaction id ${a}`);
    let i = "unknown", r = "unknown";
    for (const [s, d] of Object.entries(e.toJSON()))
      if (t.debug("getCallState - Dumping Interaction Data for interaction_id " + s), t.debug(d), s === a) {
        t.debug(`Found Interaction Data for id ${a}`);
        const n = d.interaction.owner;
        switch (t.debug(`interaction_owner is ${n}`), d.interaction.isTerminated === !0 ? i = "terminated" : n === null ? i = "wrapup" : i = d.interaction.state, r = d.type, t.debug("interaction_type = " + r), r) {
          ...
          case "AgentConsultConferenced":
            d.interaction.participants[n].consultState === "consultInitiated" && (i = "consulting");
            break;
          case "AgentWrapup":
            i = "wrapup";
            break;
  ...
  async getCallStateFromEvent(e) {
    const a = await this.getInteractionId(), i = this.agent_id, r = this.selected_task;
    t.debug("call_interaction_id = " + a), t.debug("agent id = " + i), t.debug("selected task dump:"), t.debug(r);
    let s = "unknown";
    if (a === void 0)
      t.debug("Getting Call State From Event - No call interaction id found - returning unknown");
    else {
      t.debug(`Getting Call State From Event for interaction id ${a}`);
      let d = "unknown";
      t.debug("getCallStateFromEvent - Dumping Interaction Data for interaction_id " + a);
      let n = e.data.interaction;
      t.debug(n);
      const m = n.owner;
      switch (t.debug(`interaction_owner is ${m}`), t.debug(`isTerminated = ${n.isTerminated}`), t.debug(`state = ${n.state}`), t.debug("relationshipType = " + n.callProcessingDetails.relationshipType), n.isTerminated === !0 ? s = "terminated" : m === null ? s = "wrapup" : s = n.state, t.debug("provisional call state = " + s), d = e.data.type, t.debug("interaction_type = " + d), d) {
        case "AgentConsultCreated":
          s = "consulting";
          break;
        case "AgentConsulting":
      ...
      for (const [g, l] of Object.entries(n.participants))
        t.debug("participant id = " + g), t.debug("consultState = " + l.consultState), t.debug("participant_data = " + l), l.hasLeft === !1 && l.isWrapUp === !1 && (t.debug("participant is still in call"), (l.consultState === "consulting" || l.consultState === "consultInitiated") && (t.debug("participant is consulting"), s = "consulting"));
      n.state === "connected" && n.callProcessingDetails.relationshipType === "consult" && e.data.reason !== "Consult_Transfer_To_EpDn" && (s = "consulting");
    }
    return t.debug(`*********** Returning call_state: ${s}`), s;
  }
  getEmptyButtonMap() {
    return {
      button1: {
  ...
  async getInteractionId() {
    const e = await o.actions.getTaskMap();
    for (const a of e)
      return a[1].interactionId;
  }
  async consultToDN(e) {
    t.debug("consultToDN started");
    let a = !0;
    try {
      a = this.holdParticipants;
    } catch {
      t.error("Error getting holdParticipants from config, defaulting to true.");
    }
    a = !0, t.debug("Hold Participants Setting: " + a);
    let i = await this.getInteractionId(), r = await o.agentContact.consult({
      interactionId: i,
      data: {
        destinationType: "DN",
        destAgentId: e
    ...
    return t.debug("consultToDN response: " + JSON.stringify(r)), r;
  }
  async processButtonPress(e, a) {
    switch (t.debug(`Sending button click event for button ${e} with payload ${a}`), t.debug(`button name: ${e}, payload = ${a}`), e) {
      case "button1":
      case "button2":
      case "button3":
        let i = Date.now(), r = i - this.lastButtonPressTime;
        if (this.lastButtonPressTime = i, r > 5e3) {
          t.debug(`Processing button press (${e}) - ${r}ms since last button press.`);
          let s = await this.consultToDN(a);
          t.debug(s);
        } else
          t.error(`Ignoring button press (${e}) - only ${r}ms since last button press.`);
        break;
    }
  }
  showConsultModal() {
  ...
  render() {
    ...
    t.debug("Attaching buttons to template"), this.button1_name !== "" && this.button1_dn !== "" && (t.debug("Attaching Button 1"), a += `<div class="interactionButtonHolder" id="button1_div"><md-tooltip class="md-tooltip" slot="false" placement="bottom" message="${this.button1_name}" disabled=""><md-button id='button1' arialabel="${this.button1_name}" ariahaspopup="true" size="40" maxwidth="160px" variant="secondary"><md-icon slot="icon" name="icon-headset_16"></md-icon><span class="interactionButtonText" slot="text">${this.button1_name}</span></md-button></md-tooltip></div>`, i = !0), this.button2_name !== "" && this.button2_dn !== "" && (t.debug("Attaching Button 2"), a += `<div class="interactionButtonHolder" id="button2_div"><md-tooltip class="md-tooltip" slot="false" placement="bottom" message="${this.button2_name}" disabled=""><md-button id='button2' arialabel="${this.button2_name}" ariahaspopup="true" size="40" maxwidth="160px" variant="secondary"><md-icon slot="icon" name="icon-headset_16"></md-icon><span class="interactionButtonText" slot="text">${this.button2_name}</span></md-button></md-tooltip></div>`, r = !0), this.button3_name !== "" && this.button3_dn !== "" && (t.debug("Attaching Button 3"), a += `<div class="interactionButtonHolder" id="button3_div"><md-tooltip class="md-tooltip" slot="false" placement="bottom" message="${this.button3_name}" disabled=""><md-button id='button3' arialabel="${this.button3_name}" ariahaspopup="true" size="40" maxwidth="160px" variant="secondary"><md-icon slot="icon" name="icon-headset_16"></md-icon><span class="interactionButtonText" slot="text">${this.button3_name}</span></md-button></md-tooltip></div>`, s = !0), e.innerHTML = `
        ...
        `, t.debug("JS Step 4 - attach to document"), this.shadowRoot.appendChild(e.content.cloneNode(!0)), this.updateButtons = (d) => {
      t.debug(`Creating Update Buttons Event for call state ${d}`);
      ...
      const m = this.shadowRoot.getElementById("button1"), g = this.shadowRoot.getElementById("button2"), l = this.shadowRoot.getElementById("button3"), b = this.shadowRoot.getElementById("button1_div"), h = this.shadowRoot.getElementById("button2_div"), f = this.shadowRoot.getElementById("button3_div");
      m !== null && (t.debug("Setting Button 1 State"), b.style.display = n.button1.display, m.disabled = n.button1.disabled), g !== null && (t.debug("Setting Button 2 State"), h.style.display = n.button2.display, g.disabled = n.button2.disabled), l !== null && (t.debug("Setting Button 3 State"), f.style.display = n.button3.display, l.disabled = n.button3.disabled), t.debug("Caching buttonStateMap"), this.buttonStateMap = n;
    }, i && this.shadowRoot.getElementById("button1").addEventListener("click", async (d) => {
      this.shadowRoot.getElementById("button1").disabled === !1 ? (t.debug("Button 1 Pressed. Disabling Buttons."), i && (this.shadowRoot.getElementById("button1").disabled = !0), r && (this.shadowRoot.getElementById("button2").disabled = !0), s && (this.shadowRoot.getElementById("button3").disabled = !0), await this.processButtonPress("button1", this.button1_dn)) : t.debug("Button 1 is disabled. Ignoring click event.");
    }), r && this.shadowRoot.getElementById("button2").addEventListener("click", async (d) => {
      this.shadowRoot.getElementById("button2").disabled === !1 ? (t.debug("Button 2 Pressed. Disabling Buttons."), i && (this.shadowRoot.getElementById("button1").disabled = !0), r && (this.shadowRoot.getElementById("button2").disabled = !0), s && (this.shadowRoot.getElementById("button3").disabled = !0), await this.processButtonPress("button2", this.button2_dn)) : t.debug("Button 2 is disabled. Ignoring click event.");
    }), s && this.shadowRoot.getElementById("button3").addEventListener("click", async (d) => {
      this.shadowRoot.getElementById("button3").disabled === !1 ? (t.debug("Button 3 Pressed. Disabling Buttons."), i && (this.shadowRoot.getElementById("button1").disabled = !0), r && (this.shadowRoot.getElementById("button2").disabled = !0), s && (this.shadowRoot.getElementById("button3").disabled = !0), await this.processButtonPress("button3", this.button3_dn)) : t.debug("Button 3 is disabled. Ignoring click event.");
    });
  }
...
window.customElements.define("conference-control-ui", w);
export {
  w as default,
  t as logger
};
